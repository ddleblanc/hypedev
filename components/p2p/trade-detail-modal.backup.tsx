...
export function TradeDetailModal({
  ...
  // Determine who made the last action by checking history
  // If status is COUNTERED, we need to know who made the counter-offer
  const getLastActionUser = () => {
    if (!trade || !trade.history || trade.history.length === 0) {
      // No history means original offer is still active, so initiator made last action
      return trade?.initiator.walletAddress;
    }

    // Find the most recent counter-offer or status change
    const lastHistoryEntry = trade.history[trade.history.length - 1];
    // The history includes a user object, but we need to map it to wallet address
    // We'll need to check if this user matches initiator or counterparty
    const lastActionUserId = (lastHistoryEntry as any).userId;

    if (lastActionUserId === trade.initiator.id) {
      return trade.initiator.walletAddress;
    } else if (lastActionUserId === trade.counterparty.id) {
      return trade.counterparty.walletAddress;
    }

    // Default to initiator if we can't determine
    return trade?.initiator.walletAddress;
  };

  // The user who can respond is the one who did NOT make the last action
  const canRespond = trade?.status === 'PENDING' || trade?.status === 'COUNTERED'
    ? getLastActionUser() !== address
    : false;
              ...
              <div className="space-y-6">
                ...
                {!isCounterMode && ['PENDING', 'COUNTERED'].includes(trade.status) && (
                  <div className="flex gap-3">
                    {canRespond ? (
                      /* If you can respond (you didn't make the last action), show accept/counter/reject */
                      <>
                        <Button
                      ...
                      /* If you made the last action, you can only cancel */
                      <Button
