...
export function StudioView({ setViewMode, initialView }: StudioViewProps & { initialView?: string } = {}) {
  ...
  // Initialize section from route param or URL params on mount
  useEffect(() => {
    // Priority: explicit initialView prop (from /studio/[section]) -> pathname segment -> query param
    if (initialView && ['dashboard', 'projects', 'collections', 'nfts', 'activity', 'create', 'analytics', 'settings'].includes(initialView)) {
      setCurrentView(initialView);
      return;
    }

    const path = typeof window !== 'undefined' ? window.location.pathname : '';
    const pathMatch = path.match(/\/studio\/(\w+)/);
    if (pathMatch && pathMatch[1]) {
      const section = pathMatch[1];
      if (['dashboard', 'projects', 'collections', 'nfts', 'activity', 'create', 'analytics', 'settings'].includes(section)) {
  ...
  const handleViewChange = useCallback((section: string) => {
    setCurrentView(section);
    try {
      // Navigate to real route /studio/<section> so header navigation uses real pages
      router.push(`/studio/${section}`);
    } catch (e) {
      // Fallback to updating query params for SPA path
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      */}
      ...
      {/* full-width collection detail pages are handled by real routes under /studio/collections/[id] */}
    </div>
  );
}