...
import { useDropsData, DropCollection } from "@/hooks/use-drops-data";
...
export function DropsView({ setViewMode }: DropsViewProps) {
  const router = useRouter();
  const { collections, isLoading, error } = useDropsData();
  ...
  const filteredCollections = collections.filter((collection: DropCollection) => {
    ...
    <div className="min-h-screen pt-16">
      ...
      <div className="pl-6 pr-6 lg:pl-8 lg:pr-8 py-8">
        {isLoading ? (
          ...
          >
            {filteredCollections.map((collection: DropCollection, index: number) => (
              <motion.div
                key={collection.id}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                onClick={() => router.push(`/drops/${collection.id}`)}
              >
                {viewType === 'grid' ? (
