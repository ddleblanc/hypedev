...
interface DropsProjectDetailProps {
  projectId: string;
}

// Mock project data for drop detail
// Story section type for rich editorial content
interface StorySection {
  ...
  image?: string;
  video?: string;
  layout: 'full-width' | 'left-image' | 'right-image' | 'center';
}

// Mock story sections - will be replaced with real data from collection
const mockStorySections: StorySection[] = [
  {
    id: "origin",
    title: "The Original Collection",
    description: "Every piece in this collection tells a story. Born from the intersection of art and technology, these digital artifacts represent a new era of creative expression.",
    layout: "full-width"
  },
  {
    id: "vision",
    title: "From Vision to Reality",
    description: "What started as a creative experiment has evolved into a thriving community of collectors and creators. Each piece is carefully crafted to stand the test of time.",
    layout: "left-image"
  },
  {
    id: "process",
    title: "The Creative Process",
    description: "Behind every NFT lies countless hours of iteration and refinement. Our artists push the boundaries of what's possible in the digital realm.",
    layout: "right-image"
  }
];

// Mock live mints data - will be replaced with real-time data
const mockLiveMints = [
  { id: 1, price: 0.0001, minter: "0x1234...5678", quantity: 2, timestamp: new Date(Date.now() - 60000) },
  { id: 2, price: 0.0001, minter: "0xabcd...efgh", quantity: 1, timestamp: new Date(Date.now() - 120000) },
  { id: 3, price: 0.0001, minter: "0x9876...4321", quantity: 3, timestamp: new Date(Date.now() - 180000) },
];

// Mock live sales data
const mockLiveSales = [
  { id: 1, price: 0.012, from: "0x1234...5678", to: "0xabcd...efgh", timestamp: new Date(Date.now() - 300000) },
  { id: 2, price: 0.015, from: "0x9876...4321", to: "0xfedc...ba98", timestamp: new Date(Date.now() - 600000) },
];

export function DropsProjectDetail({ projectId }: DropsProjectDetailProps) {
  ...
  // Get mint price from active phase, or upcoming phase if none active, or default to 0
  const mintPrice = activePhase
    ? (parseFloat(activePhase.pricePerToken) / 1e18)
    : upcomingPhase
      ? (parseFloat(upcomingPhase.pricePerToken) / 1e18)
      : 0;
    ...
    const checkWatchlist = async () => {
      ...
      try {
        const response = await fetch(
          `/api/lists/check?userId=${user.id}&itemType=drop&itemId=${collection.id}`
        );
        const data = await response.json();
    ...
    return (
      <div className="min-h-screen bg-black flex items-center justify-center">
        <div className="text-center">
          ...
          <Button onClick={() => router.push('/drops')} className="bg-[rgb(163,255,18)] text-black">
            Back to Drops
          </Button>
        </div>
      </div>
  ...
  const handleWatchlistToggle = async () => {
    ...
    try {
      ...
      if (isWatchlisted) {
        ...
        const response = await fetch(
          `/api/lists/items?listId=${currentWatchlistId}&itemType=drop&itemId=${collection.id}`,
          { method: 'DELETE' }
        );
        const data = await response.json();
    ...
    >
      <TooltipProvider>
        ...
        >
          ...
          {/* Hero Content */}
          <motion.div
            className="absolute bottom-0 left-0 right-0 px-4 md:px-6 pb-6 md:pb-8 max-w-7xl mx-auto"
          >
            {/* Creator Info */}
            {collection.project && (
        ...
        <div className="relative bg-black pb-24">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            {/* Sticky Tab Navigation - Only the tab bar is sticky */}
            ...
            {/* Tab Contents - Scrolls naturally */}
            <div className="px-4 md:px-6 max-w-7xl mx-auto py-8">
                  ...
                  <TabsContent value="mint" className="mt-0">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto">
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                          ...
                          {/* Thumbnail Gallery - OpenSea style */}
                            ...
                            {[...Array(4)].map((_, index) => (
                              <div
                                ...
                                className={cn(
                                  ...
                                  index === 0 ? "border-[rgb(163,255,18)]" : "border-white/10 hover:border-white/30"
                                )}
                              >
                                <MediaRenderer
                                  src={
                      ...
                      <div className="space-y-6">
                        ...
                        <Card className="bg-black/60 border-white/10 backdrop-blur-xl sticky top-16">
                          {/* Active Phase Header - OpenSea style */}
                          {activePhase && (
                            <div className="p-6 border-b border-white/10">
                              <div className="flex items-center gap-2 mb-4">
                                <span className="text-white/60 text-sm uppercase tracking-wide">
                                  {activePhase.metadata?.name || `Phase ${claimPhases?.indexOf(activePhase) + 1}` || 'Public Stage'}
                                </span>
                              </div>
                              <div className="flex items-baseline gap-2 mb-1">
                                <span className="text-white text-3xl font-bold">
                                  {mintPrice > 0 ? mintPrice.toFixed(4) : '0'}
                                </span>
                                <span className="text-white/60 text-lg">ETH</span>
                              </div>
                              {mintPrice > 0 && (
                                <p className="text-white/50 text-sm mb-3">
                                  ${(mintPrice * 3500).toFixed(2)}
                                </p>
                              )}
                              <div className="flex items-center gap-2">
                                <span className="w-2 h-2 rounded-full bg-[rgb(163,255,18)] animate-pulse" />
                                <span className="text-[rgb(163,255,18)] text-sm font-medium uppercase tracking-wide">
                                  MINTING NOW
                                </span>
                              </div>
                            </div>
                          ...
                          {/* Upcoming Phase Header */}
                          {!activePhase && upcomingPhase && (
                            <div className="p-6 border-b border-white/10">
                              <div className="flex items-center gap-2 mb-4">
                                <span className="text-white/60 text-sm uppercase tracking-wide">
                                  {upcomingPhase.metadata?.name || 'Upcoming Phase'}
                                </span>
                              </div>
                              <div className="flex items-baseline gap-2 mb-1">
                                <span className="text-white text-3xl font-bold">
                                  {(parseFloat(upcomingPhase.pricePerToken) / 1e18).toFixed(4)}
                                </span>
                                <span className="text-white/60 text-lg">ETH</span>
                              </div>
                              <div className="flex items-center gap-2 mt-3">
                                ...
                                <span className="text-orange-400 text-sm font-medium">
                                  Starts {new Date(upcomingPhase.startTimestamp).toLocaleDateString('en-US', {
                                    month: 'short',
                                    day: 'numeric',
                                    hour: 'numeric',
                                    minute: '2-digit'
                          ...
                          <CardHeader className="border-b border-white/10 pb-4 pt-4">
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <CardDescription className="text-white/50 text-sm mt-1">
                                  {collection.mintedSupply} / {collection.maxSupply || '∞'} minted
                                </CardDescription>
                              </div>
                            </div>
                          </CardHeader>
                          ...
                          <CardContent className="pt-6 space-y-6">
                            ...
                            {!isLive && isUpcoming && (!claimPhases || claimPhases.length === 0) ? (
                              ...
                              <>
                                ...
                                {/* Quantity Selector + Mint Button - OpenSea compact style */}
                                <div className="space-y-4">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex items-center bg-white/5 rounded-lg border border-white/10">
                                      <Button
                                        size="icon"
                                        variant="ghost"
                                        className="h-12 w-12 text-white hover:bg-white/10 rounded-l-lg rounded-r-none"
                                        onClick={() => setMintQuantity(Math.max(1, mintQuantity - 1))}
                                        disabled={mintQuantity <= 1}
                                      >
                                        <Minus className="w-5 h-5" />
                                      </Button>
                                      <div className="w-16 h-12 flex items-center justify-center border-x border-white/10">
                                      ...
                                      <Button
                                        size="icon"
                                        variant="ghost"
                                        className="h-12 w-12 text-white hover:bg-white/10 rounded-r-lg rounded-l-none"
                                        onClick={() => {
                                    ...
                                    <Button
                                      size="lg"
                                      className="flex-1 bg-[rgb(163,255,18)] text-black hover:bg-[rgb(163,255,18)]/90 font-bold text-base h-12"
                                  ...
                                  {/* Per Wallet Limit */}
                                  <p className="text-white/40 text-sm text-center uppercase tracking-wide">
                                    {activePhase?.quantityLimitPerWallet && activePhase.quantityLimitPerWallet > 0
                                      ? `LIMIT ${activePhase.quantityLimitPerWallet} PER WALLET`
                    ...
                    {/* Live Activity Section - OpenSea style */}
                      ...
                      <div>
                        ...
                        <Card className="bg-black/40 border-white/10">
                          <CardContent className="p-0">
                            <div className="grid grid-cols-4 gap-4 p-4 border-b border-white/10 text-xs text-white/50 font-medium uppercase">
                              <span>MINTER</span>
                              <span>PRICE</span>
                              <span>QUANTITY</span>
                              <span className="text-right">TIME</span>
                            </div>
                            {collection.mintedSupply > 0 ? (
                              mockLiveMints.map((mint) => (
                                <div key={mint.id} className="grid grid-cols-4 gap-4 p-4 border-b border-white/5 text-sm hover:bg-white/5 transition-colors items-center">
                                  <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-pink-500 flex items-center justify-center flex-shrink-0">
                                      <span className="text-white text-xs font-bold">
                                        {mint.minter.slice(2, 4).toUpperCase()}
                                      </span>
                                    </div>
                                    <span className="text-white font-medium truncate">{mint.minter}</span>
                                  </div>
                                  <span className="text-white">{mintPrice > 0 ? `${mintPrice.toFixed(4)} ETH` : 'FREE'}</span>
                                  <span className="text-white">{mint.quantity}</span>
                                  <span className="text-white/50 text-right">
                                    {Math.floor((Date.now() - mint.timestamp.getTime()) / 60000)}m ago
                                  </span>
                                </div>
                              ))
                            ) : (
                          ...
                          <CardContent className="p-0">
                            ...
                            {collection.volume > 0 ? (
                              mockLiveSales.map((sale) => (
                                <div key={sale.id} className="grid grid-cols-4 gap-4 p-4 border-b border-white/5 text-sm hover:bg-white/5 transition-colors items-center">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-white font-medium">#{sale.id}</span>
                                  </div>
                                  <span className="text-white">{sale.price.toFixed(4)} ETH</span>
                                  <span className="text-white/80">{sale.from}</span>
                                  <span className="text-white/50 text-right">
                                    {Math.floor((Date.now() - sale.timestamp.getTime()) / 60000)}m ago
                                  </span>
                                </div>
                              ))
                            ) : (
          ...
          {/* Story Sections - OpenSea editorial style */}
          <div className="mt-0">
            {/* Full-width Story Section 1 */}
            <div className="relative w-full py-24 overflow-hidden">
              ...
              <div className="relative z-10 max-w-4xl mx-auto px-6 md:px-12 text-center">
                <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-white mb-6">
                  {mockStorySections[0]?.title || `The ${collection.name} Story`}
                </h2>
                <p className="text-lg md:text-xl text-white/80 leading-relaxed max-w-3xl mx-auto">
                  {collection.description || mockStorySections[0]?.description}
                </p>
              </div>
            </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                  ...
                  <div className="space-y-6">
                    <h3 className="text-3xl md:text-4xl font-black text-white">
                      {mockStorySections[1]?.title || 'The Vision'}
                    </h3>
                    <p className="text-white/70 text-lg leading-relaxed">
                      {mockStorySections[1]?.description || 'Every collection tells a story. This one is about pushing the boundaries of digital art and creating something truly unique.'}
                    </p>
                    <div className="flex items-center gap-4 pt-4">
                      {collection.project && (
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-full bg-gradient-to-r from-[rgb(163,255,18)] to-green-400 flex items-center justify-center">
                            <span className="text-black font-bold">{collection.project.name[0]}</span>
                          </div>
                          <div>
                            <p className="text-white font-bold">{collection.project.name}</p>
            ...
            {/* Another Story Section - Reversed */}
              ...
              <div className="max-w-7xl mx-auto px-6 md:px-12">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                  ...
                  <div className="space-y-6 order-1 lg:order-2">
                    <h3 className="text-3xl md:text-4xl font-black text-white">
                      {mockStorySections[2]?.title || 'The Process'}
                    </h3>
                    <p className="text-white/70 text-lg leading-relaxed">
                      {mockStorySections[2]?.description || 'Behind every piece lies countless hours of creativity and refinement. Our commitment to quality shows in every detail.'}
                    </p>
                    <div className="grid grid-cols-2 gap-4 pt-4">
                      <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                        <p className="text-[rgb(163,255,18)] text-2xl font-bold">{collection.maxSupply?.toLocaleString() || '∞'}</p>
                        <p className="text-white/60 text-sm">Total Supply</p>
                      </div>
                      <div className="bg-white/5 rounded-lg p-4 border border-white/10">
                        <p className="text-white text-2xl font-bold">{collection.holders?.toLocaleString() || 0}</p>
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-6">
                    ...
                    <div className="flex-1 text-center md:text-left">
                      <div className="flex items-center justify-center md:justify-start gap-2 mb-1">
                        <h4 className="text-white font-bold text-lg">{collection.name}</h4>
                        <Verified className="w-5 h-5 text-blue-400 fill-current" />
                      </div>
                    ...
                    <div className="flex items-center gap-8">
                      <div className="text-center">
                        ...
                        <p className="text-white font-bold text-lg">
                          {collection.floorPrice < 0.01 ? collection.floorPrice.toFixed(4) : collection.floorPrice.toFixed(3)} ETH
                        </p>
                      </div>
                      <div className="text-center">
                        <p className="text-white/60 text-xs mb-1">TOTAL VOLUME</p>
                        <p className="text-white font-bold text-lg">
                          {collection.volume < 1 ? collection.volume.toFixed(4) : collection.volume.toFixed(2)} ETH
                        </p>
                      </div>
                    </div>
                    ...
                    <Button
                      ...
                      disabled={!isLive || isSoldOut}
                    >
                      {isSoldOut ? 'Sold Out' : isLive ? 'Mint Now' : 'Coming Soon'}
                    </Button>
                  </div>
                </CardContent>
              </Card>
        ...
        {/* Floating Mint Widget - Sticky on scroll */}
        <AnimatePresence>
          {isLive && !isSoldOut && (
            <motion.div
            ...
            >
              <Card className="bg-black/90 backdrop-blur-xl border-white/20 shadow-2xl w-80">
                <CardContent className="p-4">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="text-white font-bold text-sm truncate">{collection.name}</h4>
                      <p className="text-[rgb(163,255,18)] font-bold">
                        {mintPrice > 0 ? `${mintPrice.toFixed(4)} ETH` : 'FREE'}
                      </p>
                    </div>
                  ...
                  {collection.maxSupply && (
                    <div className="mb-3">
                      <div className="flex justify-between text-xs text-white/60 mb-1">
                        <span>{collection.mintedSupply.toLocaleString()} minted</span>
                        <span>{((collection.mintedSupply / collection.maxSupply) * 100).toFixed(0)}%</span>
                      </div>
                      <Progress
                        value={(collection.mintedSupply / collection.maxSupply) * 100}
